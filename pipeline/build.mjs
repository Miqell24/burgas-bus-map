...
// Burgas: ONE feed, and this map builds it itself — see pipeline/burgas-feed.mjs.
// 26 bus lines in the family's navy plus the single trolleybus line T1 in its
// green: Burgas ran trolleybuses from 1988, suspended them in mid-2022 and put
// T1 back under the wires on 3 February 2025. No tram, no metro, no rail here.
...
// Exact-match repairs for poles a feed names badly. Burgas needs none: the
// planner endpoint writes proper mixed-case Bulgarian, and burgas-feed.mjs has
// already dropped the transliteration each name carries behind a slash.
const NAME_FIX = {};
...
// Б lines — Burgas' quasi-BRT — open the bus lists (user 17.09.2026)
const firstRank = (k) => (/^Б/.test(k) ? 0 : 1);
const numSort = (a, b) => {
  const A = keyParts(a), B = keyParts(b);
  return firstRank(a) - firstRank(b) || A[0].localeCompare(B[0]) || (A[1] - B[1]) || A[2].localeCompare(B[2]);
};
...
//
// Burgas publishes no GTFS anywhere — the operator hands one over only against
// a signed application, the Bulgarian NAP lists its dataset with zero files,
// and OSM holds two route relations for the whole city. So the feed under
// data/gtfs is written by pipeline/burgas-feed.mjs out of the public backend of
// Burgasbus's own journey planner, which carries every pole, every line, every
// pattern and an encoded polyline for each. route_type 11 marks T1, which the
// engine already knows to paint green.
//
// allVariants: that endpoint publishes PATTERNS, not runs — 50 of them across
// 27 lines, each one a branch the operator itself lists. There is no timetable
// to sample, so there is no busiest short-turn to fall for and nothing to
// throw away; drawing all 50 is the whole network exactly once.
//
// STOP NAMES need no repair here. Burgas writes them properly cased and only
// doubles each with its transliteration ("Изгрев, бл. 3 / Izgrev, bl. 3"),
// which the feed writer strips. The seven that shout — ИЛПБ 1, СБЗ, ПГКПИ —
// shout on the pole too: they are initialisms, so nothing here touches them.
// (This is why the Varna caps dictionary is NOT carried over: its four-letter
// rewrite would have turned ИЛПБ into Илпб.)

const LBL = new Map();
const isRailTrunk = () => false;

const burgasName = (n) => String(n || '').replace(/\s+/g, ' ').trim();

const MODES = [{
  mode: 'bus', label: 'buses & trolleybus', graphMode: 'road',
  // Meden Rudnik and Slaveykov at the ends of the spine, Sarafovo and the
  // airport north-east, Kraymorie south, the villages inland — 2 × 2 tiles
  osmFiles: Array.from({ length: 4 }, (_, i) => `data/osm/tiles/t${i + 1}.json`),
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['9']),
  feeds: [
    { tag: 'bs', dir: 'data/gtfs', routeTypes: ['3', '11'], allVariants: true,
      // numbers are unique across both modes (T1 is the only trolleybus and no
      // bus is called T1), so the key is the number the flag shows, bare
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    //
    // Burgas adds a second trigger. The planner ships an OVERVIEW polyline —
